...
let rec find_by_last_name_tail_rec name db=
  let rec help name db acc =
    match db with
    | [] -> acc
    | student :: db' ->
        if student.last_name = name then help name db' (student :: acc)
        else help name db' acc
  in
  List.rev (help name db [])
